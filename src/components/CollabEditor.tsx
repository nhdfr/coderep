...
const CollabEditor = React.forwardRef(function CollabEditor(
  { docId, language = "javascript", enableVim = false, onVimReady, onVimFailed }: CollabEditorProps,
  ...
  const createVimFromModule = (mod: any, editor: any, statusNode: HTMLElement | null) => {
    // common shapes:
    ...
    try {
      ...
      if (mod.initVimMode && typeof mod.initVimMode === "function") return mod.initVimMode(editor, statusNode);
      const d = mod.default || mod;
      if (typeof d === "function") return d(editor, statusNode);
      if (d && typeof d.initVimMode === "function") return d.initVimMode(editor, statusNode);
    } catch (e) {
  ...
  const handleEditorMount = (editor: any, monaco: any) => {
    ...
    const ensureVim = async () => {
      ...
      try {
        ...
        vimModeRef.current = createVimFromModule(mod, editor, vimStatusRef.current);
  ...
  useImperativeHandle(ref, () => ({
    toggleVim: async (on?: boolean) => {
      const shouldOn = typeof on === "boolean" ? on : !Boolean(vimModeRef.current);
      if (shouldOn) {
        if (!editorRef.current) return;
        try {
          const mod = await import("monaco-vim");
          vimModeRef.current = createVimFromModule(mod, editorRef.current, vimStatusRef.current);
        } catch (e) {
          // no-op
        }
      } else {
  ...
  return (
    <div className="h-[80vh] w-full">
      <div ref={vimStatusRef} className="text-xs text-zinc-400 mb-1" />
      <Editor height="100%" defaultLanguage={language} theme="vs-dark" onMount={handleEditorMount} />
    </div>
  );
});
